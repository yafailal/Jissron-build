...
import { HorizontalBarChart } from "@/components/admin/AnalyticsChart";
...
function HeroCard({
  ...
  return (
    <div className="bg-white rounded-lg border border-line px-3.5 py-3 flex items-start gap-2.5">
      <div className="w-8 h-8 rounded-md bg-primary text-white grid place-items-center shrink-0">
        <Icon size={15} />
      </div>
...
function BreakdownPanel({
  ...
  return (
    <div className="bg-white rounded-lg border border-line p-3">
      ...
      <HorizontalBarChart data={rows.map((r) => ({ label: r.label, amountCents: r.amountCents }))} />
