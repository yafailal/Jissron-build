...
export function UsersFilters({ categories = [] }: UsersFiltersProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-line p-3 mb-3">
      <div className="flex flex-wrap items-end gap-2.5">
        <Field label={t("filters.search")} className="flex-1 min-w-[200px]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-8 rounded-full border border-line bg-white ps-8 pe-7 text-[12.5px] text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-bright/35"
            />
            {search && (
              <button
                type="button"
        ...
        {hasFilters && (
          <button
            type="button"
            onClick={reset}
            className="h-8 px-3.5 rounded-full border border-line bg-bg-soft text-[12px] font-semibold text-muted hover:bg-bg-hover hover:text-ink transition-colors"
          >
            {t("filters.reset")}
          </button>
        )}
